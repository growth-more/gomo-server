...
import com.gomo.app.core.interest.application.port.in.ReadMajorInterestUseCase;
...
public class MajorInterestApi {
	...
	public ResponseEntity<CreateMajorInterestResponse> create(@Auth AuthInfo authInfo, @PathVariable("id") UUID interestId) {
		UUID majorInterestId = createMajorInterestUseCase.create(authInfo.getPrincipalId(), interestId);
	...
	public ResponseEntity<ListMajorInterestResponse> findAll(@Auth AuthInfo authInfo) {
		List<MajorInterestDto> dtos = readMajorInterestUseCase.findAll(authInfo.getPrincipalId());
	...
	public ResponseEntity<Void> delete(@Auth AuthInfo authInfo, @PathVariable("id") UUID majorInterestId) {
		deleteMajorInterestUseCase.delete(authInfo.getPrincipalId(), majorInterestId);
