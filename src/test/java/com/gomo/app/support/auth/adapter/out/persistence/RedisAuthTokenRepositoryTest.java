...
public class RedisAuthTokenRepositoryTest {
	...
	@BeforeEach
	void setUp() {
		sut.saveRefreshToken(TOKEN_ID, "refresh_token");
	}

	@DisplayName("Redis에 저장된 refresh token을 조회 할 수 있다.")
	@Test
	void read_refresh_token() {
		assertThat(sut.findRefreshToken(TOKEN_ID)).isEqualTo("refresh_token");
	}
	...
	void delete_email_auth_code() {
		...
		assertThat(sut.findRefreshToken(TOKEN_ID)).isNullOrEmpty();
	}
}
