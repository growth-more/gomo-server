...
public class RefreshAccessTokenUseCaseTest {
    ...
    void refresh_token_successfully(){
        ...
        doReturn(REFRESH_TOKEN).when(jwtSessionRedisService).getRefreshToken(member.getId());

        doReturn(NEW_REFRESH_TOKEN).when(jwtUtil).generateRefreshToken(member.getId());
        doReturn(NEW_ACCESS_TOKEN).when(jwtUtil).generateAccessToken(member.getId());
        doReturn(EXPIRES_IN).when(jwtUtil).extractExpirationTime(anyString());

        doNothing().when(jwtSessionRedisService).updateRefreshToken(member.getId(),NEW_REFRESH_TOKEN);

        LoginMemberResponse expected = LoginMemberResponse.of(member.getId(), NEW_ACCESS_TOKEN, NEW_REFRESH_TOKEN, EXPIRES_IN);
    ...
    void refresh_token_fail_with_wrong_token(){
        ...
        doReturn(member.getId().toString()).when(jwtUtil).extractMemberId(REFRESH_TOKEN);
        doReturn("WORNG_TOKEN").when(jwtSessionRedisService).getRefreshToken(member.getId());

        assertThatThrownBy(() -> sut.refresh(REFRESH_TOKEN))
                .isInstanceOf(MemberAuthenticationFailedException.class)
                .hasMessageContaining("Member Authentication fail");
    }
