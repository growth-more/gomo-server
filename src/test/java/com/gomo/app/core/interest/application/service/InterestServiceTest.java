...
@DisplayName("[Application unit]: 관심사 서비스 테스트")
...
public class InterestServiceTest {
	...
	void read_interest() {
		...
		doReturn(Optional.of(expected)).when(interestRepository).findById(any());
		doReturn(Optional.of(majorInterest)).when(majorInterestRepository).findByInterestId(any());

		InterestDto actual = sut.read(expected.getId());
	...
	void read_interest_list() {
		...
		List<InterestDto> actual = sut.readAll(expected1.getRegistrantId());
	...
	@Test
	void read_interest_list_by_registrant_ids() {
		Interest expected1 = InterestFixture.create();
		Interest expected2 = InterestFixture.create();
		MajorInterest majorInterest = MajorInterestFixture.create(expected1.getRegistrantId(), expected1.getId());
		...
		List<InterestDto> actual = sut.readAllByRegistrantIds(Set.of(expected1.getRegistrantId(), expected2.getRegistrantId()));
	...
	void read_interest_entity() {
		...
		Interest actual = sut.readById(interest.getId());
	...
	void update_interest() {
		...
		sut.update(UpdateInterestCommand.of(interest.registrantId(), interest.getId(), "name", "#FF0000"));
	}
