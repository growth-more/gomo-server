...
class ReRollAssignQuestUseCaseTest {

	@InjectMocks
	private ReRollAssignQuestUseCase sut;
	...
	void re_roll_assign_quest() {
		...
		doReturn(questReward).when(questRewardService).find(any());

		AssignQuestDto actual = sut.reRoll(assignQuest.participantId(), assignQuest.getId());
	...
	void re_roll_assign_quest_without_quest_pool() {
		...
		doReturn(removedAssignQuest).when(assignQuestService).find(any());
