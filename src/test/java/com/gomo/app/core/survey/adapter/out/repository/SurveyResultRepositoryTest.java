package com.gomo.app.core.survey.adapter.out.repository;
...
public class SurveyResultRepositoryTest {
	...
	void create_survey_result_by_member() {
		...
		List<SurveyResult> surveyResults = List.of(
			createSurveyResult(respondentId, surveyItem1, null),
			createSurveyResult(respondentId, surveyItem2, "foo")
		);

		sut.saveAll(surveyResults);

		List<SurveyResult> actual = sut.findAllByRespondentId(respondentId);
		assertThat(actual).hasSize(2);
		assertThat(actual).usingRecursiveComparison().isEqualTo(surveyResults);
	}

	private SurveyResult createSurveyResult(UUID respondentId, SurveyItem surveyItem, String customAnswer) {
