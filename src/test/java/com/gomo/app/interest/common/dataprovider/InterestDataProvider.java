 ...
 * @ backend는 상위 관심사, spring과 java는 하위 관심사이다.
 * @ backend는 주요 관심사로 등록되어 있지 않기 때문에 주요 관심사 등록 테스트에서 사용한다.
 * @ spring과 java는 주요 관심사로 등록되어 있기 때문에 주요 관심사 등록 테스트에서 사용할 수 없다.
 */
@Component
public class InterestDataProvider {
	...
	public void initialize() {
		backend = interestRepository.findById(InterestId.of(UUID.fromString(BACKEND_ID)))
			.orElseThrow(() -> new IllegalStateException("ParentInterestFixture 초기화 실패: BACKEND_ID에 해당하는 Interest가 없습니다."));

		spring = interestRepository.findById(InterestId.of(UUID.fromString(SPRING_ID)))
			.orElseThrow(() -> new IllegalStateException("ParentInterestFixture 초기화 실패: SPRING_ID에 해당하는 Interest가 없습니다."));

		java = interestRepository.findById(InterestId.of(UUID.fromString(JAVA_ID)))
			.orElseThrow(() -> new IllegalStateException("ParentInterestFixture 초기화 실패: JAVA_ID에 해당하는 Interest가 없습니다."));
	}
